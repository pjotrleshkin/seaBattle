  ...
  if(ship.name === 'Ltype'){
    if (orientation === ORIENTATION.HORIZONTAL) {
      ...
      for(let k = x; k < x + ship.size; k++){
          field[k][y].ship = ship.index;
          field[k][y].state = CELL_STATE.SHIP;
//take last cell from loop
if (x + ship.size  - 1 === k) {
  //if ship is not located at boudaries chose "side" for last cell randomly
  if(field[y] !== 9 || field[y] !== 0){
    let rand = random(2,0);
  console.log(rand);
    if (rand === 0){
field[k][y-1].state = CELL_STATE.SHIP;
}
    else if (rand === 1) {
field[k][y+1].state = CELL_STATE.SHIP;
}
}
//in case if one ship board is at border with field place it manually
  else if (field[y] === 0) {
  field[k][y+1].state = CELL_STATE.SHIP;
}
  else if (field[y] === 9) {
...
console.log(field[x][y]);
      for(let z = y; z < y + ship.size; z++){
        field[x][z].ship = ship.index;
        field[x][z].state = CELL_STATE.SHIP;
if (y + ship.size - 1 === z){
  let rand = random(2,0);
  console.log(rand);
  if(field[x] !== 9 || field[x] !== 0){
    if (rand === 0){
      field[x-1][z].state = CELL_STATE.SHIP;
    }
    else if (rand === 1){
      field[x+1][z].state = CELL_STATE.SHIP;
    }
